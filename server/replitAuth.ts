...
export function getSession() {
  ...
  return session({
    ...
    cookie: {
      ...
      maxAge: sessionTtl,
      sameSite: "lax", // Better compatibility with localhost
    },
  });
}
...
export async function setupAuth(app: Express) {
  ...
  // Only setup OIDC if config is available (Replit Auth or other OIDC provider)
  if (config && process.env.REPLIT_DOMAINS) {
    ...
    // Local auth only - simple logout route
    app.get("/api/logout", (req, res) => {
      req.logout(() => {
        res.redirect("/");
      });
...
export const isAuthenticated: RequestHandler = async (req, res, next) => {
  ...
  // If using local auth, no need to check token expiration
